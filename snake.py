...
class Point:
    ...
    # ERRO: Faltava implementação para comparação de pontos
...
class SnakeGame:
    def __init__(self, width=640, height=480):
        ...
        self.snake = [self.head,
                      Point(self.head.x-20, self.head.y),  # ERRO: estava y-20
                      Point(self.head.x-40, self.head.y)]  # ERRO: estava y-40      
    ...
    def is_collision(self, pt=None):
        if pt is None:
            pt = self.head
        # Bateu na parede
        if pt.x >= self.width or pt.x < 0 or pt.y >= self.height or pt.y < 0:  # ERRO: Condições estavam incorretas
            return True
        # Bateu no corpo
    ...
    def _move(self, action):
        ...
        if np.array_equal(action, [1, 0, 0]):
            new_direction = clockwise_map[idx]  # Continue na mesma direção
        elif np.array_equal(action, [0, 1, 0]):
            next_idx = (idx + 1) % 4
            new_direction = clockwise_map[next_idx]  # Vire à direita
        elif np.array_equal(action, [0, 0, 1]):
            next_idx = (idx - 1) % 4
            new_direction = clockwise_map[next_idx]  # Vire à esquerda
    ...
    def play_step(self, action):
        ...
        if reward_eaten:
            ...
            reward = 0  # Pequena penalidade removida para melhor aprendizado
